...
$flash = (string) ($_SESSION['hr_request_flash'] ?? '');
$error = (string) ($_SESSION['hr_request_error'] ?? '');
unset($_SESSION['hr_request_flash'], $_SESSION['hr_request_error']);

$hrRequests = [];

try {
    $hrRequests = finance_get_hr_review_requests($pdo, ['status' => 'Pending'], 200, 0);
} catch (Throwable $e) {
    $error = 'Unable to load HR requests: ' . $e->getMessage();
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>HR - Incoming Requests</title>
...
<div class="layout">
    <main class="content" style="margin: auto; max-width: 1080px;">
        <div class="page-header">
            <h1>HR - Incoming Requests</h1>
            <p>Review the shared HR request queue used by the Financial module.</p>
        </div>
        ...
        <section class="section-card" style="margin-top: 20px;">
            <table class="notion-table">
                ...
                <tbody>
                    <?php if (empty($hrRequests)): ?>
                        <tr><td colspan="6" class="muted-cell">No pending HR requests.</td></tr>
                    <?php endif; ?>
                    <?php foreach ($hrRequests as $request): ?>
                        <tr>
                            <td>
                                <div style="font-weight: 600;"><?= htmlspecialchars((string) ($request['request_title'] ?? $request['request_details'] ?? 'HR Request'), ENT_QUOTES, 'UTF-8') ?></div>
                                <div style="color: var(--muted); font-size: 12px;"><?= htmlspecialchars((string) ($request['request_code'] ?? ''), ENT_QUOTES, 'UTF-8') ?></div>
                            </td>
                            <td><?= htmlspecialchars((string) ($request['requested_by_name'] ?? 'HR Staff'), ENT_QUOTES, 'UTF-8') ?></td>
                            <td><?= htmlspecialchars((string) ($request['department_name'] ?? 'HR'), ENT_QUOTES, 'UTF-8') ?></td>
